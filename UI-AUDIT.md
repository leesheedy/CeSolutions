...
## Update — 20 September 2026 (later): the roof, and a full layout audit

**The roof** (`src/site/roof.tsx`). The one 3D object on the site, and it is the product: a gabled section with a north-facing array, drawn on a 2D canvas with real perspective projection and flat shading, lit by the same solar position that governs the page. It sits in the estimator's left column, where it shows the mechanism behind the saving. No 3D library — Spline or three.js would have cost 300–600 KB for one moment on a site that had just shed an 823 KB video; the whole scene is about 5 KB of maths, and flat shading suits the surveyor direction better than a photoreal render. It is physically real rather than decorative: at solar noon the 22.6° array reads 99% square to the sun, which is correct for 36°S at the equinox; at dawn it reads 12% and the shadow rakes west; after dark the roof goes flat and the windows below it light up. It plays the day through once on entry, then settles on now, and respects `prefers-reduced-motion`. Five defects were found and fixed while building it: panels reading as skylights (specular lobe far too broad), the house floating, a "horizon" that was really the tilted far edge of a finite ground quad, low-sun shadows projecting above the horizon, and a camera high enough to push the sky out of frame.

**Layout audit** — 11 pages × 8 widths (320/390/430/768/1024/1280/1440/1920), full-scroll, run independently. Seventeen findings, six major; all fixed.

*Majors.* The sticky header was 94% opaque with no blur, so headlines read straight through it onto the logo — now opaque with a backdrop blur. The hero's two parallax planes were cut at different scales (`inset:-12%` vs `-4%`), giving a 7.7% size mismatch that crossfaded two offset copies of the roofline through the mask; the near plane was removed altogether, because a single flat photograph cannot carry true depth parallax without a depth map and a visible ghost is worse than no ghost. Day stamps resolved `right` against the section box, so stamps inside a `.wrap`-padded section landed 65 px further in than full-bleed ones — and the first fix made it worse, because `.wrap` is not positioned, so `right:0` shot to the viewport edge; only sections that are themselves width-constrained now take `right:0`. The work grid used `align-items:start`, so a tall feature card left a 300 px hole beside it — rows now stretch, the card body absorbs the slack and blurbs are clamped (within-row height variance is now zero). The address dropdown opened downward off the bottom of a phone screen and under the fixed dock — it now flips above the input when space is short. The sun rail's hairline cut through review cards in the full-bleed marquee — padding could not restrain a marquee whose cards translate across the whole band, so the marquee gained an edge fade, which is the right treatment for it regardless.

*Minors.* Peer section headings were rendering at six different sizes (56–70 px at 1440) because each section carried its own clamp and child-combinator rules like `.review-section > .wrap > h2` outranked a flat list; one `:is()` rule at specificity 0,3,1 now governs every section title. Headline orphans (`text-wrap: balance`, and a lower `.loc-hero h1` clamp). Dangling divider strokes in the hero trust list below 430 px. The hero kicker wrapping to two lines and stranding its corner brackets at 320. The 404 shipping `index, follow` and a generic title — now `noindex, follow` and "Page not found". Honeypot inputs as two invisible, unstyled tab stops (`inert`), and the suggestion listbox as a tab stop (`tabindex="-1"`). The collapsed day stamp colliding with the reviews kicker at 768. The mobile menu's Close button scrolling out of reach (now sticky). Footer columns wrapping while the card had slack. Footer column labels as `<h2>`, the same level as page titles — now `<h3>`.

*Confirmed clean by the same audit:* zero JS or console errors across 88 page×width loads; no horizontal overflow at any width, checked element-level rather than via `documentElement.scrollWidth` (which `body{overflow-x:clip}` would have masked); typography never falling back; breakpoint contracts honoured exactly at 1179/1180/1181 and 860/861; reduced motion handled at source rather than only in CSS; both modals properly modal with focus trapped and returned; and the three-step form working end to end with values preserved across Back.
