...
Type is **Bricolage Grotesque** (one variable file, optical size 12–96 / weight 200–800) for display and body, with **JetBrains Mono** for every number, time and coordinate; the optical-size axis does the work — opsz 96 for the hero, 72 for section heads, 48 for figures, 14 for body, 48 on phones. Colour has jobs rather than positions: **amber** marks time and measurement (`#8A5B00` on light for AA, `#FFB000` on dark), cyan `#00ADF0` is reserved for electricity (CTAs, live states), mint `#05B085` for value you keep (savings, storage, confirmations), and dark sections sit on a blue-biased `#070C11` pre-dawn ground rather than flat black. The layout follows CES's Framer project (read-only reference, see the audit): the logo's green→cyan gradient, light-blue `#DBEDF3` and cream `#FBF3EA` surfaces, black header, enquiry and process bands, pill buttons, and a corner-bracket eyebrow on section labels. The one deliberate deviation is black text on cyan buttons — the live site's white-on-cyan is 2.6:1 and fails WCAG AA. The homepage hero backdrop is a Higgsfield-generated illustration, labelled as such on the page and in `asset-sources.json`; all other photography is CES's own.
