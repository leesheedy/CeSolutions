...
export function QuoteForm(){
 ...
 <div className="simple-enquiry__grid"><label>Your name<input name="name" autoComplete="name" required maxLength={100} placeholder="Full name"/></label><label>Phone number<input name="phone" type="tel" autoComplete="tel" required maxLength={30} placeholder="Mobile or landline"/></label><label>Email <span>(optional)</span><input name="email" type="email" autoComplete="email" maxLength={254} placeholder="you@example.com"/></label><label>Suburb or postcode<input name="address" autoComplete="address-level2" required maxLength={100} placeholder="Where is your property?"/></label></div>
 <label>I’m interested in<select name="services" defaultValue="Not sure yet">{['Not sure yet','Solar panels','Solar + battery','Home battery','Commercial solar','EV charger'].map(s=><option key={s}>{s}</option>)}</select></label>
