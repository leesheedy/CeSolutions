...
{value:'Solar + battery',label:'Solar + battery',hint:'Store daytime solar for later',Icon:BatteryCharging},
{value:'Home battery',label:'Battery only',hint:'Add a battery to existing solar',Icon:BatteryCharging},
{value:'EV charger',label:'EV charger',hint:'Plan for charging at home',Icon:Car},
...
const STEPS=['Your goals','Your details','Optional extras'] as const;
...
export function QuoteForm(){
  ...
  if(state==='sent')return <div className="qf qf--done" role="status"><span className="qf__tick"><Check size={28} aria-hidden="true"/></span><h3>Thanks — it’s with the team.</h3><p>Our team will review your enquiry and contact you to discuss your property, your goals and the next step. Need to speak with us? Call <a href="tel:+61260212000">(02) 6021 2000</a>.</p></div>;
  return <form ref={formRef} className="qf" name="enquiry" method="POST" action="/?enquiry=sent" encType="multipart/form-data" data-netlify="true" data-netlify-honeypot="company" onSubmit={submit} onKeyDown={e=>{if(e.key==='Enter'&&step<2&&(e.target as HTMLElement).tagName==='INPUT'){e.preventDefault();next();}}} noValidate={false}>
    ...
    <div ref={stepRef} className="qf__steps">
      <fieldset className="qf__step" data-step="0" data-active={step===0}><legend className="qf__q">What would you like help with?<small>Choose one or more. Not sure? We’ll help you work it out.</small></legend><div className="qf__services">{SERVICES.map(s=>{const on=chosen.includes(s.value);return <label key={s.value} className={on?'svc is-on':'svc'}><input type="checkbox" name="services" value={s.value} checked={on} onChange={()=>toggle(s.value)}/><span className="svc__icon"><s.Icon size={22} strokeWidth={1.75} aria-hidden="true"/></span><span className="svc__text"><strong>{s.label}</strong><em>{s.hint}</em></span><span className="svc__check" aria-hidden="true"><Check size={14}/></span></label>;})}</div>{state==='error'&&step===0&&<p className="qf__err" role="alert">Choose a service or “Not sure yet” to continue.</p>}</fieldset>
      <fieldset className="qf__step" data-step="1" data-active={step===1}><legend className="qf__q">How can we reach you?<small>We’ll use these details to discuss your quote.</small></legend><div className="qf__grid"><label className="qf__field"><span>Your name</span><input name="name" autoComplete="name" required maxLength={100} placeholder="Full name"/></label><label className="qf__field"><span>Phone number</span><input name="phone" type="tel" inputMode="tel" autoComplete="tel" required maxLength={30} placeholder="Mobile or landline"/></label><label className="qf__field qf__field--wide"><span>Email</span><input name="email" type="email" inputMode="email" autoComplete="email" required maxLength={254} placeholder="name@example.com"/></label><AddressField/></div>{state==='error'&&step===1&&<p className="qf__err" role="alert">We need a valid email to send the checklist to.</p>}</fieldset>
      <fieldset className="qf__step" data-step="2" data-active={step===2}><legend className="qf__q" tabIndex={-1}>Add a bill or photos if you have them<small>Everything here is optional. You can request your quote now.</small></legend>
        ...
        <label className="qf__field qf__field--wide"><span>Anything else? <em>(optional)</em></span><textarea name="message" rows={3} maxLength={1500} placeholder="What would you like to change? Tell us about your bills, existing solar or future plans…"/></label>
        <div className="qf__alt">{checklist==='failed'?<><p role="alert">That didn’t send — call us on <a href="tel:+61260212000">(02) 6021 2000</a> and we’ll talk you through it.</p><button type="button" className="qf__ghost" onClick={()=>void emailChecklist()}><Mail size={16} aria-hidden="true"/>Try again</button></>:checklist==='emailed'?<p role="status"><Check size={16} aria-hidden="true"/>Checklist sent — check your inbox, then reply with the photos whenever suits.</p>:checklist==='noted'?<p role="status"><Check size={16} aria-hidden="true"/>Noted — our solar consultant will email you the list of what to send.</p>:<><p>No files handy? Request your quote below, or ask for a checklist.</p><button type="button" className="qf__ghost" onClick={()=>void emailChecklist()} disabled={checklist==='sending'}><Mail size={16} aria-hidden="true"/>{checklist==='sending'?'Sending…':'Email me a list of what to send'}</button></>}</div>
      </fieldset>
    </div>
    <div className="qf__nav">{step>0&&<button type="button" className="qf__back" onClick={()=>{setState('idle');setStep(s=>s-1);}}><ArrowLeft size={18} aria-hidden="true"/>Back</button>}{/* Distinct keys: React must not reuse the Continue button's DOM node for the submit button, or the click that advances to step 3 would also submit the form. */}{step<2?<button key="next" type="button" className="qf__next" onClick={next}>Continue<ArrowRight size={18} aria-hidden="true"/></button>:<button key="send" type="submit" className="qf__next" disabled={state==='sending'||tooBig.length>0||preparing}>{state==='sending'?'Sending…':preparing?'Preparing photos…':'Request my free quote'}<ArrowRight size={18} aria-hidden="true"/></button>}</div>
    ...
    <p className="qf__note">Free, no obligation. CES will use your details to respond to this enquiry.</p>
  </form>;
}
