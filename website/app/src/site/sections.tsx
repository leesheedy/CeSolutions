...
export function Hero(){
  ...
  // The loop is decorative and ~0.8 MB: only fetch it on wide screens that aren't asking to save data.
  const [wide,setWide]=useState(false);
  useEffect(()=>{const mq=matchMedia('(min-width:861px)');const on=()=>setWide(mq.matches&&!(navigator as Navigator&{connection?:{saveData?:boolean}}).connection?.saveData);on();mq.addEventListener('change',on);return()=>mq.removeEventListener('change',on);},[]);
  return <section ref={ref} className="hero3" aria-label="Solar and batteries in Albury-Wodonga">
    <motion.div className="hero3__bg" data-motion="" style={reduce?undefined:{y:bgY}}><img src="/assets/hero-backdrop.webp" alt="" width={2048} height={878} fetchPriority="high" decoding="async"/>{!reduce&&wide&&<video ref={video} className="hero3__video" autoPlay muted loop playsInline preload="metadata" poster="/assets/hero-backdrop.webp" aria-hidden="true"><source src="/assets/hero-loop.webm" type="video/webm"/><source src="/assets/hero-loop.mp4" type="video/mp4"/></video>}</motion.div><div className="hero3__glow" aria-hidden="true"/>
